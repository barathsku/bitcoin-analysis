...
class MassiveStocksAdapter(BaseAdapter):
    ...
    def fetch(self, api_client: Any, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            f"Fetching Massive Stocks: {ticker} from {params.get('start_date')} to {params.get('end_date')}"
        )
        ...
        while True:
            ...
            if next_url:
                ...
                response = api_client.request(method="GET", endpoint=next_url)
            else:
                # First request
            ...
            logger.info(
                f"Page {page_count}: {len(results)} records ({len(all_results)} total)"
            )
        ...
        logger.info(
            f"Completed: {ticker} - {len(all_results)} records across {page_count} pages"
        )
    ...
    def iterate_requests(
        ...
        logger.info(
            f"Massive Stocks: Requesting {ticker} from {start_date} to {end_date}"
        )
...
class MassiveForexAdapter(BaseAdapter):
    ...
    def iterate_requests(
        ...
        yield {"ticker": ticker, "start_date": start_str, "end_date": end_str, **kwargs}
